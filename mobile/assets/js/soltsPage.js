...
function pageNumberClass(trAts, pageNumber, trNum, ksNum) {
    /*alert("pageNumberClass this.id="+this.id)*/
...
function pageClickBind(trAts, page) {
    /*alert("pageClickBind page="+page)*/
    $body = $(document.body)
    $body.on('click', '#' + page, function () {
        pageNumberButton(trAts, page.substring(page.indexOf("_") + 1, page.indexOf("_") + 3));
    })
...
function pageNumberButton(trAts, page) {
    /*alert("pageNumberButton page="+page)*/
    $("#" + trAts).each(function () {
        $(this).hide();
    });
