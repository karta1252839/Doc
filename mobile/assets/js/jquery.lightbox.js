	...
	$.fn.lightbox = function(options) {
		...
		*/
		function initialize() {
			...
			var outerImage = '<div id="outerImageContainer"><div id="imageContainer"><a style="display: none; float:right;" href="javascript://" id="bottomNavClose_news"><img src="'+opts.fileBottomNavCloseImage+'"></a><iframe id="lightboxIframe"></iframe><img id="lightboxImage" /><div id="hoverNav"><a href="javascript://" title="' + opts.strings.prevLinkTitle + '" id="prevLink"></a><a href="javascript://" id="nextLink" title="' + opts.strings.nextLinkTitle + '"></a></div><div id="loading"><a href="javascript://" id="loadingLink"><img src="'+opts.fileLoadingImage+'"></a></div></div></div>';
        ...
        $('#lightbox').css('position', 'fixed')
			...
			$("#loadingLink, #bottomNavClose, #bottomNavClose_news").click(function(){ end(); return false;});
			$("#bottomNavClose").click(function(){ end(); return false;});
			$('#outerImageContainer').width(opts.widthCurrent).height(opts.heightCurrent);
			$('#imageDataContainer').width(opts.widthCurrent);
			var pcOrMp = "";
			var newsLinkVal = "";
			if($("#refHidden").val() != "" && typeof($("#refHidden").val()) != "undefined" && $("#refHidden").val() != null){
				var RefHidden = $("#refHidden").val();
			}
			$.ajax({
				type : "post",
				...
				success : function(data) {
					if (data) {
						if(data.success == true) {
							pcOrMp = data.pcOrMp;
							if (data.havaNewsOrNot == true) {
								newsLinkVal = data.advertisingList[0].link;
							}
						} else {
						}
					}
			...
			if (!opts.imageClickClose) {
				if (newsLinkVal == null || newsLinkVal == "") {
					$("#lightboxImage").click(function(){ return false; });
				} else {
					if (pcOrMp == 'PC') {
						if (newsLinkVal.indexOf("/app/") != -1) {
							if(newsLinkVal.indexOf("?") != -1){
								$("#lightboxImage").click(function(){ window.location.href=newsLinkVal + "&ref=" + RefHidden});
							} else {
								$("#lightboxImage").click(function(){ window.location.href=newsLinkVal + "?ref=" + RefHidden});
							}
						} else {
							$("#lightboxImage").click(function(){ window.open(newsLinkVal)});
						}
					} else {
						if (newsLinkVal.indexOf("/app/") != -1) {
							if (newsLinkVal.indexOf("/app/home") != -1 || newsLinkVal.indexOf("/app/joinNow") != -1) {
								if(newsLinkVal.indexOf("?") != -1){
									$("#lightboxImage").click(function(){ window.location.href=newsLinkVal + "&ref=" + RefHidden});
								} else {
									$("#lightboxImage").click(function(){ window.location.href=newsLinkVal + "?ref=" + RefHidden});
								}
							} else if (newsLinkVal.indexOf("/app/promotion") != -1) {
								...
								var endIndex;
								if (newsLinkVal.indexOf("?") != -1){
									endIndex = newsLinkVal.lastIndexOf("?");
								} else {
									endIndex = newsLinkVal.lenght;
								}
								var newsForPage = newsLinkVal.substring(newsLinkVal.lastIndexOf("/"),endIndex);
								$("#lightboxImage").click(function(){ window.location.href=newsLinkVal.replace(newsForPage,"/loginPage" + "?ref=" + RefHidden)});
							}
						} else {
							$("#lightboxImage").click(function(){ window.open(newsLinkVal)});
						}
		...
		*/
		function showLightbox(imageObject) {
			...
			**/
			...
			var lightboxTop = arrayPageScroll[1] + ($(window).height() / 7);
			var lightboxLeft = arrayPageScroll[0];
			$('#lightbox').css({top: lightboxTop+'px', left: lightboxLeft+'px'}).show();

			if (!opts.slideNavBar) {
				$('#imageData').hide();
				$('#bottomNavClose_news').show();
			}
		...
		$.fn.lightbox.defaults = {
		  triggerEvent: "click",
			allSet: false,
			fileLoadingImage: '../images/all/lightbox/loading.gif',
			fileBottomNavCloseImage: '../images/all/lightbox/closelabel.gif',
			overlayOpacity: 0.6,
			borderSize: 10,
