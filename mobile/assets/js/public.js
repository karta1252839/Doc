        ...
        success: function (data) {
            if (data) {
                if (data.success == false) {
                    ...
                    bind_loginGame(isLogin, data.client)
                }
            } else {
            }
        },
...
function bindOnclickPublic(isLogin) {
    if (isLogin) {
        ...
        $('#slotsPpGameBind').bind("click", function () { playGameSlots('SLOTS_PP', 'vs1024butterfly'); });
        ...
        $("div[data-GameType=SLOTS_ALLBET]").bind("click", function () { playGameWait('SLOTS_ALLBET', 'af'); });
        ...
        /*99kap手机进游戏提示*/
        $('#liveAllbetGameBind99').bind("click", function () { JqueryShowMessageJustClose('Coming Soon.'); });
        $('#electroicAllbetGameBind99').bind("click", function () { JqueryShowMessageJustClose('Coming Soon.'); });
...
function bind_loginGame(isLogin, client) {
            ...
            if (gameType != null) {
                if (gameType == "LOTTERY_IG" || gameType == "LOTTO_IG" || gameType == "LOTTERY_PJ" || gameType == "LOTTO_PJ" || gameType == "GFC_IG" || gameType == "LOTTERY_IG_VR") {
                    ...
                    return;
                        ...
                        javascript: window.location.href = '../app/slotsGame?sgt=SLOTS';
                    return;
                } else if (gameType == 'SLOTS_PT') {
                        javascript: window.location.href = '../app/slotsGame?sgt=SLOTS_PT';
                    return;
                } else if (gameType == 'SLOTS_SPADE') {
                        javascript: window.location.href = '../app/slotsGame?sgt=SLOTS_SPADE';
                    return;
                } else if (gameType == 'SLOTS_AG') {
                        javascript: window.location.href = '../app/slotsGame?sgt=SLOTS_AG';
                    return;
                } else if (gameType == 'SLOTS_MG') {
                        javascript: window.location.href = '../app/slotsGame?sgt=SLOTS_MG';
                    return;
                } else if (gameType == 'SLOTS_BBIN') {
                        javascript: window.location.href = '../app/slotsGame?sgt=SLOTS_BBIN';
                    return;
                } else if (gameType == 'SLOTS_HB') {
                        javascript: window.location.href = '../app/slotsGame?sgt=SLOTS_HB';
                    return;
                } else if (gameType == "jsBindNo") {
                    return;
                }

                playGameCreditPop(gameType, gameWallet, "MAIN_WALLET", "", client);
            }
        } else {
            if (client == "MP") {
                $('.bind-loginGame').bind("click", function () { javascript: window.location.href = '../app/loginPage' });
            }
        }
...
function textBlur(textId) {
    $("#" + textId).blur();
}

function playGameCreditPop(gameType, gameWalletIn, gameWalletOut, slotsCode, client) {
    $("#gamebtn").unbind();
    $("#creditBtn").unbind();
    $('#creditPop').show();

    $('#creditPop-close').bind("click", function () {
        $("#creditPop").hide();
        $("#text_prompts").html("");
        $("input[id=txtAmount]").val("");
    })

    $("div[class=credit_chip] span").click(function () {
        var amout = $("input[id=txtAmount]").val();
        var amoutCredit = $(this).attr("data-amount");
        var cOutWalletAmout = $("#cOut_wallet_amount").html();
        if (amoutCredit.indexOf(".") != -1) {
            cOutWalletAmout.slice(0, amoutCredit.indexOf("."));
        }
        if (amoutCredit == "ALL") {
            var amount_total = parseInt(cOutWalletAmout);
        } else if (amout === "" || amout == 0) {
            var amount_total = parseInt(amoutCredit);
        } else {
            if ((parseInt(amout) + parseInt(amoutCredit)) < parseInt(cOutWalletAmout)) {
                var amount_total = parseInt(amout) + parseInt(amoutCredit);
            } else {
                var amount_total = parseInt(cOutWalletAmout);
            }
        }
        $("input[id=txtAmount]").val(amount_total);
        textBlur("txtAmount");
    })

    $("span[id=amount_empty]").click(function () {
    ...
    obj.walletId = "";
    objMain.walletId = "";
    getWalletIdOrName(gameWalletIn, obj);
    getWalletIdOrName(gameWalletOut, objMain);
    $("input[name=creditOut]").val(objMain.walletId);
    $("input[name=creditIn]").val(obj.walletId);

    $("#cOut_wallet_name").html(l_wallet[gameWalletOut]);
    $("#cIn_wallet_name").html(l_wallet[gameWalletIn]);
    $("#cOut_wallet_amount").attr("class", gameWalletOut);
    $("#cIn_wallet_amount").attr("class", gameWalletIn);

    if (obj.walletId == "close") {
        $("#cIn_wallet_amount").html("钱包维护中...");
    } else {
        getBalanceByClass(gameWalletIn);
    }

    if (objMain.walletId == "close") {
        $("#cOut_wallet_amount").html("钱包维护中...");
    } else {
        getBalanceByClass(gameWalletOut);
    }

    $(".creditPop_refresh_out").bind("click", function () {
        getWalletIdOrName(gameWalletOut, obj);
        if (obj.walletId == "close") {
            $("#cOut_wallet_amount").html("钱包维护中...");
        } else {
            getBalanceByClass(gameWalletOut);
    ...
    $(".creditPop_refresh_in").bind("click", function () {
        getWalletIdOrName(gameWalletIn, obj);
        if (obj.walletId == "close") {
            $("#cIn_wallet_amount").html("钱包维护中...");
        } else {
    ...
    $("#credit_switch").bind("click", function () {
        $("input[id=txtAmount]").val("");
        playGameCreditPop(gameType, gameWalletOut, gameWalletIn);
    })
    ...
    $("#gamebtn").bind("click", function () { //进入游戏
        if (gameType != null) {
            if (gameType == 'FISH_GG') {
                ...
                cacheImg('lottoPJ');
            }/* else if (gameType == "SLOTS" || gameType=='SLOTS_PT' || gameType=='SLOTS_SPADE' || gameType=='SLOTS_AG' || gameType=='SLOTS_MG' || gameType=='SLOTS_BBIN' || gameType=='SLOTS_HB') {
				playGameSlots(gameType,slotsCode);
			} */else if (gameType == 'SPORT') {
			    if (client == "MP") {
			        playGameMobile('SPORT');
			    } else {
			        javascript: window.location.href = '../app/sportsGame';
			    }
			} else if (gameType == 'SPORT_IWC') {
			    if (client == "MP") {
			        playGameMobile('SPORT_IWC');
			    } else {
			        javascript: window.location.href = '../app/sportsGameIwc';
			    }
			} else if (gameType == 'SPORT_CMD') {
			    if (client == "MP") {
			        playGameMobile('SPORT_CMD');
			    } else {
			        javascript: window.location.href = '../app/sportsGameCmd';
			    }
			} else {
			    playGame(gameType);
			}
        }
        $('#creditPop-close').click();
    })
}
...
$(function () {
    if (site == "xinLangCaiPiao") {//新浪国际特有方法
        ...
        if (isLogin) {
            $('#VIP').bind("click", function () { window.open('http://qycpvip.com/'); });
        }
    } else if (site == "tangChenYL") {//汤臣娱乐
        $('#kuaisuchongzhi1').unbind();
        $('#kuaisuchongzhi2').unbind();
                ...
                success: function (result) { //返回数据根据结果进行相应的处理
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        htmlText +=
		                    ...
		                    '<img src="../images/CN/aoMenTYC/pc/vs.png">' +
		                    ' </div>' +
		                    '<div class="bit-right">' +
		                    '<div class="right-img">' +
    ...
    } else if (site == "leFuCP" || site == "leFuCPDL") {//乐福彩票
        $('#liaotian').unbind("click");
        $('#vip').unbind("click");
        $('#huodongDT').unbind("click");
        $('#liaotian').bind("click", function () { javascript: window.open('http://lfcp9999.com/'), 'blank' });
        $('#vip').bind("click", function () { javascript: window.open('http://654679.com/', 'blank') });
        $('#huodongDT').bind("click", function () { javascript: window.open('http://693794.com/wap', 'blank') });

    } else if (site == "caiYouHui") {//彩友会
        if (webUrl.indexOf("6661558.com") || webUrl.indexOf("9527111.com")) {
            $("body").append('<script type="text/javascript" src="https://js.users.51.la/19538649.js"></script>');
        }
    } else if (site == "daZhuangJiaCP") {//大庄家
        if (webUrl.indexOf("664965.com")) {
            $("body").append('<script language="javascript" src="http://count28.51yes.com/click.aspx?id=286205167&logo=1" charset="gb2312"></script>');
        }
    } else if (site == "aoMenYinHeYL") {//澳门银河
        if (webUrl.indexOf("99.yinh9.com")) {
            $("body").append('<script language="javascript" src="http://s13.cnzz.com/z_stat.php?id=1274059515" charset="gb2312"></script>');
        }
    } else if (site == "wanHaoCP" || site == "wanHaoCPDL") {//万豪彩票
        /*document.write(unescape("<span id='cnzz_stat_icon_1274065217'></span>" +
				"<script src='"+cnzz_protocol+"s19.cnzz.com/z_stat.php?id=1274065217&show=pic' type='text/javascript'></script>"));*/
        $("body").append('<script src="http://s19.cnzz.com/z_stat.php?id=1274065217&show=pic" charset="gb2312"></script>');
    } else if (site == "xinTianXiaYL" || site == "xinTianXiaYLDL") {//信天下
        if (webUrl.indexOf("xtx2651.com")) {
            $("body").append('<script language="javascript" src="https://hm.baidu.com/hm.js?268882e4a724fb95cd821900de2f81ed"></script>');
        }
    } else if (site == "hongTuGJ") {//宏图国际
        $(function () {
            var obj = document.getElementById("Sam3");
            ...
            setInterval(function () {
                if (flag == 0) {
                    ...
                    base = base - parseInt(Math.random() * 10);
                    flag = 0;
                }
        ...
        $(function () {
            var obj = document.getElementById("Sam4");
            ...
            setInterval(function () {
                if (flag == 0) {
                    base1 += parseInt(Math.random() * 10);
                    flag = 1;
                } else {
                    base1 = base1 - parseInt(Math.random() * 10);
                    flag = 0;
                }
        ...
        $(function () {
            var obj = document.getElementById("Sam5");
        ...
        /*$(function() {
			...
			setInterval(function() {
				if (flag == 0) {
					base2 += parseInt(Math.random() *100);
					flag = 1;
				} else {
					base2 = base2 - parseInt(Math.random() *100);
					flag = 0;
				}
		...
		*/
    } else if (site == "hongLiCP" || site == "hongLiCPDL") {//红利彩票手机版单独进游戏方法
        $('#hongbao').unbind();
        $('#hongbao').bind("click", function () { window.open('http://kstd.me/?qqdrsign=056ed'); });
    } else if (site == "jiuZhouCP" || site == "jiuZhouCPDL") {//九州彩票
        $('#qianghongbao').unbind("click");
        $('#qianghongbao').bind("click", function () { javascript: window.open('http://www.jzcphby.com/'), 'blank' });

    } else if (site == "jiLiCP") {//吉利彩票
        $('#palyGame-bjsc-MP').bind("click", function () { playGameMobile('LOTTERY_IG', '2', 'default'); });
        $('#palyGame-cpssc-MP').bind("click", function () { playGameMobile('LOTTERY_IG', '1', 'default'); });
