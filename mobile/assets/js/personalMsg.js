...
function deleteMoreMsg() {
    if (confirm(l_perMsg['SELECT_MSG_DELETE'])) {
        var deleteMoreMsg = "";
...
function deletePersonalMsg(obj) {
    if (confirm(l_perMsg['DELETE_YES_NO'])) {
        var deId = obj;
        var o = {
            deleteId: deId
        };
                        ...
                        for (var i = 0; i < data.resultList.length; i++) {
							...
							trHtml += 
								"<div class='mes_tit'>"+
									...
									"<img src='../images/all/icon_close2.png'/></a>"+
								"</div>"+
...
//									  "<td >"+ l_perMsg[data.resultList[i].type] + "</a></td>"+
                        ...
                        if (pageNum == 1) {
                            if (pageAllNumber != 1) {
                                $("#pageNext").click(function () {
                                    getPersonalMsgForMobile((pageNum + 1));
                                });
                            }
                        } else if (pageNum == pageAllNumber) {
                            $("#pagePrev").click(function () {
                        ...
                        for (var j = forBegin; j <= forEnd; j++) {
                            if (j != pageNum) {
                                pageNumHtml += "<a onclick=\"getPersonalMsgForMobile(" + j + ");\" style='cursor: pointer;'>" + j + "</a>&nbsp;";
                            } else {
                                pageNumHtml += "<font class='F_bold ' style='color:#f00; font-weight: bold;'>" + j + "</font>&nbsp;";
                            }
                        }
        ...
        success: function (data) {
            if (data) {
                if (data.success == true && data.returnResult == 1) {
                    ...
                    //			                "</p>"+
                    //			                "<a class='close' href='javascript:void(0);' onclick='deletePersonalMsg(\""+msgId+"\")' data-ajax='false'><img src='../images/all/icon_close2.png'></a>"+
                    //			            "</div>"+
