...
function firstCharacter(field, rules, i, options) {
    if (!checkFirstCharacter()) {
        return "* " + l_register['msg20'];
    }
}
...
function fixedPhone(field, rules, i, options) {
    if ($("#txtPhoneNum").val().length != 11) {
        return "* 请输入固定11位数";
    }
}
...
function checkFirstCharacter() {
    ...
    $.ajax({
        type: "post",
        url: $("#path").val() + "/app/register/checkFirstCharacter",
        data: jsonuserinfo,
        contentType: 'application/json',
        dataType: "json",
        async: false,
            ...
            success: function (data) {
                if (data) {
                    if (data.success == true) {
                        ...
                        if ($("#tianXi_resi").length > 0) {
                            $("#tianXi_resi").hide();
                        }
                        if ($("#huaSheng_reg").length > 0) {
                            $("#huaSheng_reg").hide();
                        }
                        JqueryShowMessageWithRedirect(l_register['msg4'], "home?l=0&ref=" + data.referal);
                    } else {
                ...
                if (data) {
                    if (data.success == true) {
                        ...
                        if (data.message == 'CHECK_CODE') {
                            ...
                            JqueryShowMessage(l_register['msg16']);
                        } else if (data.message == 'REFERRAL_AGENT_NO_RECOMMEND') {
                            JqueryShowMessage(l_register['msg23']);
                        }
                    }
                }
            },
...
/*用户名判断都是小写*/
function userNameLowercaer(field, rules, i, options) {
    var str = field.val();
    var result = str.match(/^.*[A-Z]+.*$/);
    if (result != null) {
        return "* " + l_register['msg21'];
    }
}
...
function checkPassWord(field, rules, i, options) {
    ...
    if (strs1 != null) {
        for (var i = 0; i < strs1.length; i++) {
            if (strs1[i].length > 3) {
                if (Num1.indexOf(strs1[i]) != -1) PWD_Legality = false;//数字顺序
                if (Num2.indexOf(strs1[i]) != -1) PWD_Legality = false;//数字倒序
            }
        }
    }
    strs1 = t_PWD_str.match(rex2);
    if (strs1 != null) {
        if (strs1.length == 1) {
            if (strs1[0].length == 1) PWD_Legality = false;
        }//只有一个字母
    }
    for (var i = 0; i < t_PWD_str.length - 2; i++) {
        if (t_PWD_str.charAt(i) == t_PWD_str.charAt(i + 1)) {
    ...
    if (pwd == "123abc") PWD_Legality = false;
    if (pwd == "abc123") PWD_Legality = false;
    if (PWD_Legality == false) return "* " + l_register['msg22'];
}
